...
export abstract class Handle {
   ...
   * storage proxy
   */
  disable(storage: Store, particle?: Particle): void {
    if (storage instanceof StorageProxy) {
      storage.deregister(particle, this);
    }
    ...
    this.storage = StorageProxy.newNoOpProxy(this.storage.type);
  }
